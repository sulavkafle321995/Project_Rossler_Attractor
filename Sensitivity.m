...
%--------------------------------------------------------------------------
x0 = [-0.04; -0.3; 0.2];    % first initial condition
[t,x] = ode45(f,tspan,x0);
plot(t,x(:,1),'-b*','MarkerSize',2); hold on
%--------------------------------------------------------------------------
x1 = [-0.045; -0.3; 0.2];   % second initial condition
[t,x] = ode45(f,tspan,x1); 
plot(t,x(:,1),'-r','LineWidth',1);
hold all;
xlabel('t'); ylabel('x(t)');
legend(sprintf('x(t) with I.C [%.3f, %.1f, %.1f]',x0),...
    sprintf('x(t) with I.C [%.3f, %.1f, %.1f]',x1));

...
% FP is the fixed point =

%--------------------------------------------------------------------------
function Attractor(a,b,c,x0)
[t,x] = Rossler(a,b,c,x0);     % the function Rossler1() will solve the system
plot3(x(1,:),x(2,:),x(3,:),'-r'); hold all
%  view(2)
FP = FixedPoint(a,b,c);     % the function will return the fixed point of the system
plot3(FP(1,1),FP(1,2),FP(1,3),'b*'); hold all   % 1st fixed point
plot3(FP(2,1),FP(2,2),FP(2,3),'b*'); hold all   % 2nd fixed point
